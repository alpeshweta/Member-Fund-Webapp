...
export function TdpDashboardPage() {
  const { state } = useLocation()
  const navigate = useNavigate()
  const fund = (state as { fund?: TdpProduct } | null)?.fund
  ...
  const expectedYears = meta?.source_years_tdp ?? []
      ...
      <div className="mx-auto max-w-xl px-4 py-10 space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="space-y-0.5">
            ...
            <p className="text-sm text-slate-600">{fund.investment_menu_name}</p>
