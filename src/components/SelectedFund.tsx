...
export function SelectedFund({ fund, onClear, onNavigate }: Props) {
  return (
    <div className="mt-4 bg-white border border-slate-200 rounded-lg shadow-sm p-4 space-y-3">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-base font-semibold text-slate-800">{fund.product_name}</p>
        </div>
        <button
          type="button"
          onClick={onClear}
          className="min-h-[44px] min-w-[44px] flex items-center text-sm text-slate-500 underline hover:text-slate-700 shrink-0"
